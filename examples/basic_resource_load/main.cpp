...
int main() {
	...
	std::shared_ptr<Smasher::ShaderResource> pShader = engine.GetResourceManager().GetOrLoadResource<Smasher::Manifest::Shaders::basic_texture_shader, Smasher::ShaderResource>();
	...
	image.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 600.f, 300.f })
		.SetScale(sf::Vector2f{ 400.f, 400.f })
	...
	clipTest.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 180.f, 330.f })
	...
	shape.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 400.f, 50.f })
	...
	for (int i = 0; i < numEntities; ++i) {
		float depth = (float)(std::rand() % 100) / 100.f;
	...
	window1.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 400.f, 200.f })
		.SetScale(sf::Vector2f{ 150.f, 150.f })
	...
	window2.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 420.f, 220.f })
		.SetScale(sf::Vector2f{ 150.f, 150.f })
		.SetTextureAsset<Smasher::Manifest::Textures::window>({.transluscent = true})
		//.SetClipRect(sf::IntRect{ 0, 0, 64, 64 })
		.SetDepth(0.5);
	...
	inWindowEntity.AddComponent<Smasher::DrawableComponent>()
		.SetPosition(sf::Vector2f{ 410.f, 210.f })
		.SetScale(sf::Vector2f{ 100.f, 100.f })
