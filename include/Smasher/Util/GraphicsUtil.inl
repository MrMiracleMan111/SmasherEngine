...
namespace Smasher {
	namespace GraphicsUtil {
		...
		template<class T>
		GPUBlockPool<T>::GPUBlockPool(std::shared_ptr<SDL_GPUDeviceWrapper> gpu, int blockSize, int blockCount, std::string debugName) :
			m_BlockSize(sizeof(T)* blockSize* blockCount),
			m_DebugName(debugName)
		{
			bool hasDebugName = m_DebugName.compare("") != 0;

			SDL_GPUBufferCreateInfo instanceBufferInfo{};
			instanceBufferInfo.size = m_BlockSize;
			instanceBufferInfo.usage = SDL_GPU_BUFFERUSAGE_VERTEX | SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ;
			m_BlockBuffer = SDL_CreateGPUBuffer(gpu->Get(), &instanceBufferInfo);

			if (hasDebugName) {
				SDL_SetGPUBufferName(gpu->Get(), m_BlockBuffer, m_DebugName.c_str());
			}
